...
df_queries = pd.read_excel(EXCEL_INPUT_FILE, engine='xlrd')
...
if uploaded_files:
    ...
    if selected_pdf:
        ...
        # --- Start of caching embeddings logic ---

        # Check if we already cached embeddings for this PDF
        if 'current_pdf' not in st.session_state or st.session_state.current_pdf != selected_pdf:
            ...
            # Store chunks in session_state for reuse
            st.session_state.table_chunks1 = table_chunks1
            st.session_state.table_chunks2 = table_chunks2
            st.session_state.docs = docs

            # Generate embeddings once and cache them
            st.write("Generating embeddings for the uploaded PDF, please wait...")
            st.session_state.table_chunks1_embeddings = generate_google_embeddings(table_chunks1)
            st.session_state.table_chunks2_embeddings = generate_google_embeddings(table_chunks2)
            st.session_state.docs_embeddings = generate_google_embeddings(docs)
            st.write("Embedding generation complete.")

        else:
            # Use cached chunks and embeddings
            st.write("Using cached embeddings for the uploaded PDF.")
            table_chunks1 = st.session_state.table_chunks1
            table_chunks2 = st.session_state.table_chunks2
            docs = st.session_state.docs

        def search_google_embeddings(query, chunks, chunk_embeddings, top_k):
            """
            Search for the most relevant chunks using precomputed embeddings.
            """
        ...
        for index, row in df_queries.iterrows():
            ...
            for q in query1:
                try:
                    st.write(f"Running search for query: {q}")

                    retrieved_table_rows_stream = search_google_embeddings(q, st.session_state.table_chunks1, st.session_state.table_chunks1_embeddings, top_k1)
                    retrieved_table_rows_lattice = search_google_embeddings(q, st.session_state.table_chunks2, st.session_state.table_chunks2_embeddings, top_k1)
                    retrieved_text_chunks = search_google_embeddings(q, st.session_state.docs, st.session_state.docs_embeddings, top_k)

                    combined_results.extend(retrieved_table_rows_stream)
                    combined_results.extend(retrieved_table_rows_lattice)
                    combined_results.extend(retrieved_text_chunks)
